...
export var _handleInit = function() {
    ...
    baseLogList.map(method => {
        window.console[method] = (...args) => {
            //是否打印
            if (isPrint && _self.console[method] && Tools.isFunction(_self.console[method])) {
    ...
    window.console.timeEnd = function(label) {
        ...
        if (pre) {
            ...
            //是否打印
            if (isPrint && _self.console.log && Tools.isFunction(_self.console.log)) {
