...
export var _getReportContent = function(params:reportParams) {
    ...
    if (!params || !params.type || !params.typeName || !params.data) {
        consoleTools.$devLog('[keepObserver] reportServer receive reportData is not right : typeName and type and data is undefined ')
        return false;
    }
    //是否是开发模式需要打印
    if (develop) {
        var log = tool.extend({}, params)
        consoleTools.$devLog('[keepObserver] get' + log.type + 'type:' + log.typeName + " of monitor data")
    }
    //上报
