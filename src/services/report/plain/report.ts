...
export var _handleReport = function(params, control) {
    ...
    //获得上传数据
    var reportData = _self._createReportData(params, control)
        //上传到服务器
    var {
        reportUrl,
        onReportFail,
    ...
    reportUrl.map(function(item) {
        ...
        _self._handleHook(onReportBeforeHook, reportData, reportConfig.url, reportConfig.customeHead);
        //上传到服务器
        try {
            AjaxServer(reportConfig).then(function(result:any) {
                //response data
                _self._handleResponse(params,control,url,result.data)
                //hook
